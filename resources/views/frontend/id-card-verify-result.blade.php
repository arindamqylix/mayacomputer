...
@section('title', $verified ? 'ID Card Verified - Maya Computer Center' : 'ID Card Verification - Maya Computer Center')
    ...
    <div class="verify-card">
        @if($verified)
            <div class="verify-card-header verified">
                <div class="verify-icon"><i class="fa fa-id-card"></i></div>
                <h1>Verified as ID Card</h1>
                <p>This student ID card has been verified by Maya Computer Center</p>
            </div>
            <div class="verify-card-body">
                ...
                <div class="verify-detail-row">
                    <span class="verify-detail-label">Father's Name</span>
                    <span class="verify-detail-value">{{ strtoupper($data->sl_father_name ?? '') }}</span>
                </div>
                <div class="verify-detail-row">
                    <span class="verify-detail-label">Course{{ !empty($data->course_names) && str_contains($data->course_names, ',') ? 's' : '' }}</span>
                ...
                <div class="verify-detail-row">
                    ...
                    <span class="verify-detail-value">{{ format_dob_display($data->sl_dob) }}</span>
                </div>
                @endif
        ...
        @else
            ...
            <div class="verify-card-body">
                ...
                <p class="mb-0 mt-3 text-muted small">The registration number may be incorrect, or the student ID may not be active yet. Please contact Maya Computer Center.</p>
                <div class="verify-cta">
                    <a href="{{ route('index') }}">Back to Home</a>
                </div>
            </div>
