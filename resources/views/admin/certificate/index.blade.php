									...
									<tbody>
										...
										@foreach($certificates as $cert)
											<tr>
												...
												<td>
													<span
														class="badge-status status-{{ strtolower($cert->sc_status ?? 'generated') }}">
														{{ $cert->sc_status ?? 'GENERATED' }}
													</span>
													@if(isset($cert->sc_type) && $cert->sc_type == 'TYPING')
														<br><span class="badge bg-danger mt-1" style="font-size: 0.65rem;">TYPING</span>
													@endif
												</td>
												<td>
													<a href="{{ route('admin.view_certificate', $cert->sc_id) }}" class="btn-view"
													...
													<a href="{{ route('admin.delete_certificate', $cert->sc_id) }}"
														class="btn-delete" title="Delete Certificate"
