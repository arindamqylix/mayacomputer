		...
		<div class="card modern-card">
			<div class="card-header page-header">
				<div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
					...
					<div class="d-flex gap-2">
						<a href="{{ route('edit_student', $student->sl_id) }}" class="btn btn-light btn-sm">
							<i class="fas fa-user-edit me-1"></i> Edit Profile
						</a>
						<a href="{{ route('student_list') }}" class="btn-back btn-sm">
			...
			<div class="card-body">
								...
								<tr>
									...
									<td>
										@if($enrollments->count() > 1)
											<a href="{{ route('admin.student.courses.remove', ['id' => $student->sl_id, 'courseId' => $enrollment->course_id]) }}"
												class="btn-remove"
				...
				<div class="add-course-box">
					...
					@else
						<form action="{{ route('admin.student.courses.add', $student->sl_id) }}" method="POST" class="row g-3 align-items-end">
							@csrf
							<div class="col-md-8">
...
$(document).ready(function() {
	$('.enrollment-status-select').on('change', function() {
		...
		$.ajax({
			url: '{{ route('admin.student.courses.status', $student->sl_id) }}',
			type: 'POST',
			data: {
				_token: '{{ csrf_token() }}',
				course_id: courseId,
