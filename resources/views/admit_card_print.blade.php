    ...
    <div class="admit-card">
        ...
        <div class="header">
            ...
            <div class="header-center">
                ...
                <div class="admit-title">प्रवेश पत्र (ADMIT CARD) – {{ date('Y') }}</div>
            </div>
            <div class="header-right">
                <div class="qr-code">
                    @if(!empty($admit->reg_no))
                        <img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data={{ url('verify-admit-card/' . ($admit->ac_id ?? '')) }}" alt="QR Code" style="width:70px;height:70px;">
        ...
        <!-- Registration Row -->
        <div class="reg-row">
            <div>Registration No.: <span>{{ $admit->reg_no }}</span></div>
            <div>Registration Years: <span>{{ \Carbon\Carbon::parse($admit->created_at)->format('Y') }}</span></div>
        </div>
        ...
        <div class="student-section">
            <div class="student-details">
                <div class="detail-row">
                    ...
                    <div class="detail-value">{{ strtoupper($student->sl_name) }}</div>
                </div>
                <div class="detail-row">
                    <div class="detail-label">Father's Name:</div>
                    <div class="detail-value">{{ strtoupper($student->sl_father_name ?? 'N/A') }}</div>
                </div>
                <div class="detail-row">
                    <div class="detail-label">Mother's Name:</div>
                    <div class="detail-value">{{ strtoupper($student->sl_mother_name ?? 'N/A') }}</div>
                </div>
                ...
                <div class="detail-row">
                    ...
                    <div class="detail-value">{{ $center->cl_code ?? '' }}- {{ strtoupper($center->cl_name ?? $center->cl_center_name ?? 'N/A') }}</div>
                </div>
                <div class="detail-row">
                    <div class="detail-label">Course Name:</div>
                    <div class="detail-value">{{ $course->c_full_name ?? 'N/A' }}</div>
                </div>
                <div class="detail-row">
                    <div class="detail-label">Course Duration:</div>
                    <div class="detail-value">{{ $course->c_duration ?? 'N/A' }}</div>
                </div>
            </div>
        ...
        <div class="note-section">
            ...
            @if($admit->exam_notice)
                {{ $admit->exam_notice }}
            @else
                Any kind of specific identifying marks made by student in the Answer Book is subject to non evaluation / or shall be treated as <span>Unfairmeans</span>. Bringing of Calculators / Phone or any other electronic gadget in side the examination hall shall be deemed as <span>Unfairmeans</span> & breach of examination rules.
            @endif
        </div>
