...
@section('title', 'Payment for Document Reissue')
@push('custom-css')
<style type="text/css">
	.payment-header {
				...
				<div class="card-body p-4">
					...
					<div class="payment-summary text-center">
						...
						<p class="mb-0">
							@if($reissueRequest->drr_document_type == 'CERTIFICATE')
								<i class="fa-solid fa-certificate"></i> Certificate Reissue
							@elseif($reissueRequest->drr_document_type == 'MARKSHEET')
								<i class="fa-solid fa-file-lines"></i> Marksheet Reissue
							@else
								<i class="fa-solid fa-id-card"></i> ID Card Reissue
							@endif
						</p>
					</div>
					...
					<div class="payment-info-card">
						...
						<div class="payment-info-row">
							...
							<span class="payment-info-value"><strong>₹ {{ number_format($reissueRequest->drr_amount, 2) }}</strong></span>
						</div>
					</div>
						...
						<div class="col-md-8 mx-auto">
							<div class="card">
								<div class="card-body text-center">
									...
									<form action="{{ route('student.document_reissue.payment.process', $reissueRequest->drr_id) }}" method="POST" id="razorpayForm">
										@csrf
										...
										<script
											...
											data-amount="{{ $reissueRequest->drr_amount * 100 }}"
											data-currency="INR"
											data-order_id="{{ $orderId }}"
											data-buttontext="Pay ₹ {{ number_format($reissueRequest->drr_amount, 2) }}"
											data-name="Maya Computer Center"
											data-description="Document Reissue - {{ $reissueRequest->drr_document_type }}"
											...
											data-prefill.notes.request_id="{{ $reissueRequest->drr_id }}"
										></script>
									</form>
								</div>
							</div>
