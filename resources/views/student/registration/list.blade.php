...
@section('title', 'Registration Cards')
@push('custom-css')
<style type="text/css">
	.doc-list-page {
		--dl-primary: #2563eb;
			...
			<div class="page-card">
				<div class="list-header">
					<div class="list-header-inner">
						<h4>
							<span class="header-icon"><i class="fas fa-address-card"></i></span>
							My Registration Cards
							@if($enrollments->count() > 0)
								<span class="count-badge">{{ $enrollments->count() }} {{ $enrollments->count() === 1 ? 'course' : 'courses' }}</span>
							@endif
						</h4>
						<p class="subtitle">View and print your official registration card for each enrolled course</p>
					</div>
				</div>
				<div class="card-body">
					@if(session('error'))
					...
					<div class="info-banner">
						...
						<span>Your registration number is the same for all courses. Select a course below to view or print its registration card.</span>
					</div>
									...
									<tbody>
										...
										@foreach($enrollments as $row)
											...
											<tr>
												...
												<td>
													@if($canView)
														<a href="{{ route('student.view_registration_card_detail', $row->course_id) }}" class="btn-view" target="_blank">
					...
					@else
						<div class="empty-state">
							<div class="empty-icon"><i class="fas fa-address-card"></i></div>
