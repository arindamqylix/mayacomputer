...
<style type="text/css">
    ...
    .status-badge.verified { background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%); }
    .status-badge.pending { background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%); }
    .status-badge.dispatched { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); }
    .status-badge.block { background: linear-gradient(135deg, #ef4444 0%, #b91c1c 100%); }
    .status-badge.other { background: linear-gradient(135deg, #64748b 0%, #475569 100%); }
			...
			<div class="card-body p-0">
						...
						<table id="datatable-buttons" class="table modern-table table-hover mb-0">
							...
							<tbody>
								@foreach($student as $data)
									<tr>
										<td><strong>#{{ $data->sl_id }}</strong></td>
										...
										<td>
                                            @php
                                                $statusClass = 'other';
                                                $s = strtoupper($data->sl_status);
                                                if($s == 'VERIFIED') $statusClass = 'verified';
                                                elseif($s == 'PENDING') $statusClass = 'pending';
                                                elseif($s == 'DISPATCHED') $statusClass = 'dispatched';
                                                elseif($s == 'BLOCK') $statusClass = 'block';
                                            @endphp
											<span class="status-badge {{ $statusClass }}">{{ $data->sl_status ?? 'N/A' }}</span>
										</td>
										<td>
				...
				@else
					<div class="text-center p-5">
						<i class="fas fa-users" style="font-size: 4rem; color: #6c757d; opacity: 0.5;"></i>
						<h5 class="mt-3">No Students Found</h5>
						<p class="text-muted">You haven't registered any students yet.</p>
					</div>
				@endif
			</div>
		</div>
