...
class InvoiceController extends Controller
{
    ...
    public function downloadTransactionInvoice($id)
    {
        ...
        $transaction = DB::table('transaction')
            ->where('t_id', $id)
            ->where('t_FK_of_center_id', $centerId)
            ->first();
        ...
        // Generate invoice/receipt number (Using transaction ID for simplicity or generate like others)
        ...
        $pdf = PDF::loadView('center.invoice.transaction_invoice_pdf', $data);
        $pdf->setPaper('A4', 'portrait');
