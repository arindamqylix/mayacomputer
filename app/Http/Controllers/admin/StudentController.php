...
class StudentController extends Controller
{
	public function student_list(Request $request)
	{
		...
		$query = DB::table('student_login as s')
			...
			->select(
				...
				DB::raw('MAX(s.sl_FK_of_center_id) as sl_FK_of_center_id'),
		...
		$studentRows = $query->get();
		foreach ($studentRows as $row) {
			$row->course_names = student_course_names(
				(string) $row->sl_reg_no,
				(int) $row->sl_FK_of_center_id
			);
		}
		$student['student'] = $studentRows;
	...
	/**
	 * Manage all course enrollments for one student on a dedicated page.
	 */
	public function student_courses($id)
	{
		...
		return view('admin.student.enrollments', [
			'student' => $student,
			'center' => $center,
	...
	public function add_student_course(Request $request, $id)
	{
		...
		try {
			...
			return redirect()->route('admin.student.courses', $id)
	...
	public function remove_student_course($id, $courseId)
	{
		...
		try {
			...
			return redirect()->route('admin.student.courses', $redirectId ?: $id)
	...
	/**
	 * Unified enrollment rows for one student (reg no + center).
	 */
	private function getStudentEnrollmentsList(string $regNo, int $centerId): \Illuminate\Support\Collection
	{
	...
	private function attachCourseToStudent(Student $student, int $courseId, Center $center): void
	{
		$regNo = $student->sl_reg_no;
		$centerId = (int) $student->sl_FK_of_center_id;
	...
	private function detachCourseFromStudent(object $student, int $courseId): void
	{
		$regNo = $student->sl_reg_no;
		$centerId = (int) $student->sl_FK_of_center_id;
	...
	private function deleteCourseSpecificRecords(int $slId, int $courseId): void
	{
		DB::table('set_result')
